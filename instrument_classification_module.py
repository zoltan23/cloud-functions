...
def classify(filename):
    print('instrument_classification_module.classify called')
    ...
    print('Original audio file min~max range:', np.min(
        scipy_audio), 'to', np.max(scipy_audio))
    print('Librosa audio file min~max range:', np.min(
        librosa_audio), 'to', np.max(librosa_audio))
    ...
    y=librosa_audio, sr=librosa_sample_rate, n_mfcc=40)
    print(mfccs.shape)
    ...
    checkpointer = ModelCheckpoint(filepath='/tf/desktop/Audio-Classification/saved_models/weights.best.basic_mlp.hdf5',
    ...
    predicted_instrument = print_prediction(filename)
    return predicted_instrument